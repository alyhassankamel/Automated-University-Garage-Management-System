-- AUGMS_SOFT Database Schema for Microsoft SQL Server
...
-- Entry/Exit Log table for vehicle check-in/check-out
...
CREATE INDEX IX_EntryExitLogs_LicensePlate ON EntryExitLogs(license_plate);
CREATE INDEX IX_EntryExitLogs_SpotNumber ON EntryExitLogs(spot_number);
CREATE INDEX IX_EntryExitLogs_Timestamp ON EntryExitLogs(timestamp);
CREATE INDEX IX_Vehicle_LicensePlate ON Vehicle(license_plate);
CREATE INDEX IX_Parking_spots_Status ON Parking_spots(spot_status);
CREATE INDEX IX_Parking_spots_GarageID ON Parking_spots(garage_id);
...
BEGIN
    INSERT INTO Parking_garage (name, garage_access, garage_status, location)
    VALUES ('Main Garage', 1, 'Open', 'Campus Building A');
END
GO

-- Insert sample parking spots (20 spots)
DECLARE @i INT = 1;
DECLARE @garage_id INT = 1;

WHILE @i <= 20
BEGIN
    IF NOT EXISTS (SELECT 1 FROM Parking_spots WHERE spot_number = 'SPOT' + RIGHT('000' + CAST(@i AS VARCHAR), 3))
    BEGIN
        INSERT INTO Parking_spots (spot_number, spot_status, garage_id)
        VALUES ('SPOT' + RIGHT('000' + CAST(@i AS VARCHAR), 3), 'AVAILABLE', @garage_id);
    END
    SET @i = @i + 1;
END
GO
...
PRINT 'Sample parking garage and spots inserted.';
