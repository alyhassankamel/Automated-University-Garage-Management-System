...
try {
    $stmt = $pdo->query("SHOW COLUMNS FROM Service_request LIKE 'payment_method'");
    if ($stmt->rowCount() == 0) {
        $pdo->exec("ALTER TABLE Service_request ADD COLUMN payment_method ENUM('Credit', 'Cash', 'Pending') DEFAULT 'Pending'");
        $pdo->exec("ALTER TABLE Service_request ADD COLUMN payment_amount DECIMAL(10,2) DEFAULT NULL");
        $pdo->exec("ALTER TABLE Service_request ADD COLUMN card_number VARCHAR(20) DEFAULT NULL");
        $pdo->exec("ALTER TABLE Service_request ADD COLUMN card_cvv VARCHAR(4) DEFAULT NULL");
        $pdo->exec("ALTER TABLE Service_request ADD COLUMN card_expiry VARCHAR(5) DEFAULT NULL");
        $pdo->exec("ALTER TABLE Service_request ADD COLUMN cardholder_name VARCHAR(100) DEFAULT NULL");
        $pdo->exec("ALTER TABLE Service_request ADD COLUMN payment_status ENUM('Pending', 'Paid', 'Failed') DEFAULT 'Pending'");
    }
} catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $request['status'] == 'Approved' && ($request['payment_status'] ?? 'Pending') != 'Paid') {
    ...
    if ($payment_method == 'Credit') {
        ...
        if (empty($card_number) || empty($card_cvv) || empty($card_expiry) || empty($cardholder_name)) {
            ...
            // Ensure receipt_id exists, generate if not
            ...
            $stmt = $pdo->prepare("UPDATE Service_request SET 
                                  ...
                                  receipt_id = ?
                                  WHERE request_id = ?");
            if ($stmt->execute([$amount, $card_number, $card_cvv, $card_expiry, $cardholder_name, $receipt_id, $request_id])) {
                ...
                $stmt->execute([$request_id, $user_id]);
                $request = $stmt->fetch(PDO::FETCH_ASSOC);
                $request['receipt_id'] = $receipt_id; // Ensure receipt_id is set
            } else {
        ...
        $stmt = $pdo->prepare("UPDATE Service_request SET 
                              ...
                              receipt_id = ?
                              WHERE request_id = ?");
        if ($stmt->execute([$amount, $receipt_id, $request_id])) {
